...
Purpose
------
This README describes how to use the project files in a typical folder. It explains what each file does, what inputs are expected, how to run the analyses, and where outputs are produced.
...
------
R (recent version) with the following packages:
  survival, stats, gridExtra, broom, tidyr, stringr, dplyr, reshape2, ggplot2, ggpubr, survminer, openxlsx
...
3) Phenotype table keyed by sample ID with ADRD-related binary columns (e.g., EOAD, LOAD, DLB, VD, etc.). These are typically collapsed into a single binary outcome (dementia_present). Essentially this is phenotype file with sample id and associated phenotype as the two columns of the file. One sample can have multiple phenotypes, which will correlate to multiple rows in the phenotype table.

4) Covariate table keyed by Sample (or sample_id) including GENDER/SEX, AGE_OF_ENTRY, and principal components (PC1->PC10). AGE_OF_ENTRY is the age of the visit where they were diagnosed with the ADRD phenotype.
...
A) Regression File Creation.Rmd
   ...
   - Merges covariates and phenotypes; collapses phenotypes to dementia_present. If they have an ADRD phenotype -> they are 1 on the dementia_present column. Otherwise they are 0.
...
B) Multiple Regression.Rmd (Odds Ratios)
   - For each variant, fits: glm(dementia_present ~ carrier + GENDER + AGE_OF_ENTRY + PC1..PC10, family = binomial).
...
C) Cox Regression.Rmd (Hazard Ratios)
   ...
   - For each variant, fits: coxph(Surv(AGE_OF_ENTRY, dementia_present) ~ carrier + GENDER + PCs).
...
1) Build the dataset
   ...
   - The final output of this code will be a long-type dataframe that will be saved as csv that can be used for the regression analysis
...
3) Run Cox models (HRs)
   ...
   - Ensure the merged dataset and APOE status are available (or load them from saved CSVs).
   - Run to produce results and the forest plot.
...
Notes
------
- If you encounter path errors, replace setwd(...) or absolute paths with project-relative paths.
- If outputs are clipped in plots, adjust plot limits or enable arrowheads for off-scale confidence intervals as shown in the code comments.
